/* ============================================================
   ...
   · Zalora Phase 1  — entities SG/MY/HK/ID/PH, currencies,
     languages (English/Mandarin/Bahasa/Filipino), Freshdesk
...
// The 7-stage processing pipeline (Vendor Queries PRD §7.1)
const PIPELINE = [
  { k: "A", icon: "📥", c: "var(--primary-500)", title: "Intake",
    desc: "Every inbound vendor email becomes a structured Query record — reusing the Zalora Freshdesk channel. One message can carry several questions, each a resolvable intent (VQ-A1.4). Whatever can't be parsed isn't dropped — it lands in the Ingestion log for review, which is what the Coverage metric measures." },
  ...
  { k: "C", icon: "🧩", c: "var(--purple-500)", title: "Context Assembly",
    desc: "Resolve the legal entity & currency (SG/MY/HK/ID/PH), then fetch the invoice, PO, GRN/SES, payment document, Accounting Entry Posting ID and tax amounts into one context object. Every ERP/SAP read goes through a connector you wire up in Integrations — the seam each client plugs into. Wrong entity = wrong answer." },
  { k: "D", icon: "🎯", c: "var(--primary-500)", title: "Attempt to Resolve",
    desc: "No mandatory classification — like P2P Smart Routing, the system simply tries to ground a confident answer and scores it against gates G2–G4. The only question that matters: can I answer this confidently, or not?" },
  { k: "E", icon: "✳️", c: "var(--green-500)", title: "Outcome",
    desc: "Auto-answers reply on their own the moment the ticket arrives — no human trigger — if grounded AND high-confidence. Otherwise the query routes to a human, who opens a summarized view with the records, a suggested next step and an editable suggested reply to send. Bank-detail changes are captured & routed, never auto-applied (Epic I)." },
  { k: "J", icon: "✉️", c: "var(--green-600)", title: "Reply & Close",
    desc: "A single consolidated reply on the same email thread, in the vendor's language, tracked to SLA. The Freshdesk ticket status is synced back (Neoflo - Auto-resolved / Routed / …). A post-close vendor reply re-opens the same query to a human (VQ-A2)." },
  { k: "K", icon: "📊", c: "var(--grey-700)", title: "Log & Learn",
    desc: "Immutable audit of every query, the auth_check, records used, resolution path and outbound reply — so containment and accuracy can be measured, and driver analysis feeds the P2P / procurement roadmap (Epic K)." },
];
...
const SCENARIOS = [
  ...
  },
  {
    ...
    flag: "🇵🇭", lang: "Filipino", entity: ENTITIES[4], amount: "632.00",
  ...
  },
  {
    ...
    flag: "🇮🇩", lang: "Bahasa Indonesia", entity: ENTITIES[3], amount: "8,750,000",
    ...
    run: [
      { state: "ok",   k: "Intake · 3 intents", d: "One message → three resolvable intents (VQ-A1.4) · via Freshdesk FD-88210" },
      { state: "ok",   k: "G1 · Identity passed", d: "ar@fjdigital.co.id → authorised contact · language: Bahasa Indonesia" },
      ...
      { state: "warn", k: "Intent 3 · Tax certificate", d: "WHT/PPh certificate isn't in any system we read → routed (manual in v0, VQ-E4)" },
      { state: "info", k: "One consolidated reply", d: "Answerable intents sent now; routed intent acknowledged — never held back (VQ-J4)" },
    ],
    ladder: ladder("cleared"),
    answer: {
      lead: "2 of 3 answered now; 1 with our team.",
      body: "<strong>1) INV-5567</strong> — dibayar pada 5 Juni 2026, ref TXN-55901.<br><strong>2) Bukti pembayaran INV-5512</strong> — terlampir (payment-proof-2000905910.pdf).<br><strong>3) Sertifikat PPh Q1</strong> — sedang diproses oleh tim kami; Anda akan menerima kabar dalam SLA.",
      stamp: "Reply mirrors the vendor's language (Bahasa Indonesia). One outbound email, not three (VQ-J4).",
    },
  },
];
...
const QUERIES = [
  ...
  { id: "VQ-4810", ref: "FD-88210", vendor: "FJ DIGITAL DESIGN SERVICES", email: "ar@fjdigital.co.id",
    ...
    reason: "Tax certificate · manual fetch (v0)", sla: "−0h 18m", slaState: "breach", assignee: "Unassigned",
...
const UNPARSED = [
  ...
  { id: "RAW-9910", ts: "11 Jun · 08:03", sender: "billing@trans-asia.vn", subject: "Hỏi về thanh toán hóa đơn",
    ref: "FD-88238", reason: "Unsupported language (Vietnamese) — could not classify", cls: "failed", recoverable: true },
...
const CONNECTORS = [
  { group: "Channel", name: "Freshdesk", desc: "Vendor email intake + ticket status sync (Zalora pattern)",
  ...
  { group: "Data sources", name: "Tax certificate portal", desc: "WHT/TDS certificate source — manual in v0, automation is v1",
...
const ANALYTICS = {
  metrics: [
    { label: "Containment rate", value: "78.4%", delta: "+6.1", up: true, sub: "In-scope queries resolved with no human · last 30 days", c: "var(--green-500)" },
    { label: "Auto-answer accuracy", value: "96.2%", delta: "+0.8", up: true, sub: "Weekly QA sample · near-guardrail", c: "var(--primary-500)" },
    { label: "Median time to answer", value: "1m 41s", delta: "−12s", up: true, sub: "Auto-answered · intake → reply sent", c: "var(--purple-500)" },
    { label: "Coverage / ingestion", value: "93.0%", delta: "+1.4", up: true, sub: "Inbound messages parsed into a structured query", c: "var(--orange-500)" },
  ],
  byType: [
    ...
    { label: "Tax certificate (manual v0)", pct: 0, c: "var(--orange-500)" },
    { label: "Dispute / short-pay", pct: 0, c: "var(--red-500)" },
  ],
  mix: [
    { label: "Payment status", pct: 42, c: "var(--primary-500)" },
    { label: "Document request", pct: 19, c: "var(--purple-500)" },
    { label: "Payment breakdown", pct: 14, c: "var(--green-500)" },
    { label: "Dispute / short-pay", pct: 11, c: "var(--orange-500)" },
    { label: "Tax (cert + rate)", pct: 9, c: "var(--yellow-500)" },
    { label: "Bank change / other", pct: 5, c: "var(--grey-400)" },
  ],
  drivers: [
